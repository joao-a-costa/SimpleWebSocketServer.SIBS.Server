...
if exist "%configFile%" (
    ...
    echo Reading port and service name from config.ini...
    ...
    echo Service name loaded from config.ini: !serviceName!
) else (
:askPort
...
:askServiceName
    set /p "userServiceName=Enter service name (leave empty for default): "
    if "!userServiceName!"=="" (
        set "serviceName=SmartCASLESS - Server - !port!"
